...
import { fetchApprovedOrders, fetchApprovedOrdersByClientAndType } from "../api/orderApi";

const SearchOrder = ({ onSearchResults, resetSearchTerm, setResetSearchTerm, selectedTab }) => {
  ...
  const searchByClientName = async () => {
    ...
    if (selectedTab === "ALL") {
      ...
      return approvedOrders.filter(order => 
        matchingClients.some(client => client.clientCode === order.clientCode)
      );
    }

    // SALE/PURCHASE 탭일 경우 해당 타입의 주문만 조회
    return await fetchApprovedOrdersByClientAndType(
      matchingClients[0].clientCode, 
      selectedTab
    );
  };
  ...
  const searchByEmployeeName = async () => {
    ...
    // ALL 탭일 경우 모든 주문 조회
    if (selectedTab === "ALL") {
      return approvedOrders.filter(order => employeeIds.includes(order.employeeId));
    }

    // SALE/PURCHASE 탭일 경우 해당 타입의 주문만 조회
    return approvedOrders.filter(order => 
      employeeIds.includes(order.employeeId) && order.orderType === selectedTab
    );
  };
