...
export const getAttendanceByEmployee = async (employeeId) => {
  try {
    const response = await axiosInstance.get(`/api/attendances/employee/${employeeId}`);
    return response.data;
  } catch (error) {
...
export const getAttendancesByStatus = async (status) => {
  try {
    const response = await axiosInstance.get(`/api/attendances/attendance/status/${status}`);
    return response.data;
  } catch (error) {
...
export const saveAttendance = async (attendanceData) => {
  try {
    const response = await axiosInstance.post("/api/attendances", attendanceData);
    return response.data;
  } catch (error) {
...
export const updateAttendanceStatus = async (id, newStatus) => {
    try {
      const response = await axiosInstance.put(`/api/attendances/${id}/status`, null, {
        params: { status: newStatus },
      });
      return response.data;
    } catch (error) {
      console.error(`❌ 근태 상태 업데이트 실패 (ID: ${id}, 상태: ${newStatus}):`, error);
      throw error;
    }
  };
  ...
  export const requestApproval = async (id, status, date, reason) => {
    try {
        const response = await axiosInstance.put(`/api/attendances/${id}/request-approval`, null, {
            params: { // ✅ 요청 파라미터 방식으로 전달
                status, // ✅ 근태 유형
                date,   // ✅ 신청 날짜
                reason  // ✅ 신청 사유
            }
        ...
        console.error(`❌ 승인 요청 실패 (ID: ${id}):`, error.response?.data || error.message);
        throw error;
    }
};
