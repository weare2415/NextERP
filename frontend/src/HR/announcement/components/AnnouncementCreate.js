...
const AnnouncementCreate = ({ onClose, onAddSuccess  }) => { // ✅ onClose 부모에서 전달받음
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setAnnouncement({ title: "", content: "", authorId: employeeId, departmentId: null, positionId: null, isGlobal: false });
    ...
    return (
        <div className="modal-overlay" onClick={onClose}> {/* ✅ 모달 바깥 클릭 시 닫기 */}
            <div className="modal-content" onClick={(e) => e.stopPropagation()}> {/* ✅ 내부 클릭 시 닫히지 않음 */}
                <h3>📢 공지사항 생성</h3>

                {!employeeId ? (
                    <p className="error-message">⚠️ 로그인이 필요합니다.</p>
                ) : !isAuthorized ? (
                    <p className="error-message">⚠️ 공지사항을 작성할 권한이 없습니다. (차장 이상만 가능)</p>
                ) : (
                    <form onSubmit={handleSubmit}>
                        <input type="text" name="title" placeholder="제목" value={announcement.title} onChange={handleChange} required />
                        <textarea name="content" placeholder="내용" value={announcement.content} onChange={handleChange} required />

                        {/* ✅ 전체 공지사항 체크박스 */}
                        {isGlobalAnnouncement && (
                            <label>
                                <input type="checkbox" checked={announcement.isGlobal} onChange={handleCheckboxChange} />
                                전체 공지사항으로 등록 (이사 이상만 가능)
                            </label>
                        )}

                        {/* ✅ 로그인된 사용자 정보 자동 입력 (보이지만 수정 불가) */}
                        <input type="hidden" name="authorId" value={announcement.authorId} readOnly />
                        <input type="hidden" name="departmentId" 
                        value={announcement.departmentId ? departmentMap[announcement.departmentId] || "전체" : "전체"} 
                        readOnly />

                        <input type="hidden" name="positionId" 
                        value={announcement.positionId ? positionMap[announcement.positionId] || "N/A" : "N/A"} 
                        readOnly />

                        <div className="button-group">
                            <button type="submit">공지 생성</button>
                            <button type="button" onClick={onClose} className="cancel-btn">취소</button> {/* ✅ 취소 버튼 추가 */}
                        </div>
                    </form>
                )}
            </div>
