    ...
    const handleSearch = async () => {
        try {
            ...
            if (!searchTerm.trim()) {
                ...
                // ✅ 제목, 작성자 검색
                const filteredResults = allAnnouncements.filter(announcement =>
                ...
                if (departmentId) {
                    ...
                    const departmentResults = await getAnnouncementsByDepartment(Number(departmentId)).catch(() => []);
                ...
                if (positionId) {
                    ...
                    const positionResults = await getAnnouncementsByPosition(Number(positionId)).catch(() => []);
                    searchResults = [...searchResults, ...positionResults];
                }
            }

            // ✅ 중복 데이터 제거
            const uniqueResults = Array.from(new Set(searchResults.map(a => a.id)))
