...
const BasicLayout = ({ children }) => {
	...
	const [user, setUser] = useState({ name: "", positionTitle: "", departmentName:"" });
	...
	const menuItems = {
		...
		운영관리: [
		  ...
		  { name: '판매/구매 승인 요청', path: '/product/request' }
		]
	  };
        ...
        <div className="basic-layout">
            <div className="main-layout">
                <nav className="sidebar">
                    <div className="logo">
                        <img src="/NextERP_layoutLogo.png" alt="Logo" onClick={handleGoHome} />
                    </div>
                    ...
                    <ul>
                        {Object.entries(menuItems).map(([category, items]) => (
                            <li key={category}>
                                <div 
                                    className={`menu-item ${openMenu === category ? 'active' : ''}`}
                                ...
                                {openMenu === category && (
                                    <ul className="submenu">
                                        {items.map((item) => (
                                            <li key={item.path} className={location.pathname === item.path ? "active-submenu" : ""}>
