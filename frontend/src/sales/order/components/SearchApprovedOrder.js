...
const SearchApprovedOrder = ({ onSearchResults }) => {
	...
	const handleSearch = async (e, page = 0) => {
		...
		try {
			// 선택한 페이지의 트랜잭션 가져오기
			const transactions = await getTransactionByDateBetween(startDate, endDate, page, size);
	...
	return (
			<div className="search-approved-order">
				<form className="search-form" onSubmit={(e) => handleSearch(e, 0)}>
					<label>날짜 선택:</label>
