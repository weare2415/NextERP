...
import "./scss/ApprovedOrderPage.scss";
...
const ApprovedOrderPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchNames = async () => {
      const clientCodes = [
        ...new Set(mergedOrders.map((order) => order.clientCode)),
      ].filter(Boolean);
      const productIds = [
        ...new Set(mergedOrders.map((order) => order.productId)),
      ].filter(Boolean);
