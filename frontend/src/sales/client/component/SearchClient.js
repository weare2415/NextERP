...
const SearchClient = ({ onSearchResults }) => {
	...
	const fetchSuggestions = async (query, searchType) => {
		try {
			...
			if (searchType === 'clientCode') {
				...
				data = response.content.map(client => ({ clientCode: client.clientCode }));
			} else if (searchType === 'clientName') {
				const response = await getClientByName(query);
				data = response.content.map(client => ({ clientName: client.clientName }));
			}
			setSuggestions(data);
		} catch (error) {
	...
	const handleKeyDown = (e) => {
		...
		if (e.key === 'ArrowDown') {
			...
			const selectedValue = selectedItem?.clientCode || selectedItem?.clientName; // 안전한 접근
