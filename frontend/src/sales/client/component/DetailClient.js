...
const DetailClient = ({
  ...
  const generateMemo = (originalData, modifiedData) => {
    ...
    Object.keys(originalData).forEach((key) => {
      if (originalData[key] !== modifiedData[key] && key !== "memo") {
        // memo 필드는 비교 제외
        const fieldName = fieldNames[key] || key;
        changes.push(
          `${fieldName} 변경 (${originalData[key] || "없음"} → ${
            modifiedData[key] || "없음"
          })`
        );
      }
  ...
  return (
    <div className="modal-overlay">
      <div className="client-detail-form">
        ...
        <form className="client-detail-grid">
          ...
          <div className="form-group">
            ...
            <input
              ...
              onChange={handleChange}
            />
            <button type="button" className="zipCode-btn">
              우편번호 찾기{" "}
            </button>
          </div>
          ...
          <div className="form-group">
            ...
            <textarea
              name="memo"
              value={userMemo + generateMemo(client, clientData)}
              onChange={handleChange}
            ></textarea>
          </div>
